# Module 6 — MAPPO learner: PPO-clip actor + clipped-value critic with KL early-stop.
...
class MAPPOLearner:
    ...
    def update(self, batch, state_graph_embed=None):
        c = self.cfg
        feats = batch["feats"]; masks = batch["masks"]; anchors = batch["anchors"]
        actions = batch["actions"]; old_logps = batch["logps"]
        adv_agent = batch["adv_agent"]; adv_state = batch["adv_state"]
        ...
        for _ in range(c.epochs):
            ...
            for start in range(0, n, mb_size):
                ...
                logp, ent = self.actor.evaluate(
                    feats[mb_t], masks[mb_t], actions[mb_t], anchors[mb_t])
